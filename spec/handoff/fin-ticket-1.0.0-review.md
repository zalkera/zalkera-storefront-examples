...
> ⚠️ **이 표는 그때(client `0.27.0`)로 잰 기록이다.** 다시 돌릴 수 있는 명령이 적힌 행은
> 첫 행 하나이고, 나머지 여덟은 **방법 서술**이라 그대로는 못 잰다.
> 「테마 주입」 행이 그렇다 — 확인 방법이 「서명 대조」인데, 대조할 서명이 지금은 없다:
>
> ```bash
...
> ```
>
> 그래서 그 행은 **기록**이지 「지금도 그래야 한다」가 아니고, 이 납품본처럼 그 배선을 가진
> 소스는 손봐야 한다. 기록을 덮어쓰지 않는 이유가 그것이다 — 덮으면 그 사실의 거처가 사라진다.
