...
**안 쓰는 능력의 입구(메뉴·링크·진열)를 닫는 것**입니다 — 파일은 지우지 않습니다. 어디를 닫으면 되는지는
[`AGENTS.md`](AGENTS.md) 의 "안 쓰는 능력" 절에 있습니다. 헤더 같은 표현은 아예 자기 것으로 다시 쓰면 됩니다.
...
| `NEXT_PUBLIC_{GOOGLE,NAVER,BING}_SITE_VERIFICATION` | 검색엔진 소유확인 토큰(HTML 태그 방식의 `content` 값). 비우면 그 태그가 안 나간다 |
