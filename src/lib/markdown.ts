 ...
 * (실제로 그 둘은 이 축과 무관했고, 마크다운 DON'T-BUILD 는 **고정 페이지** 축이었다).
 * 죽은 인용이었다.
 *
 * ■ 왜 라이브러리를 안 쓰는가
 *
 ...
 *
 * 🔴 **개수로 세면 가장 싼 노드가 예산을 정한다.** 개수로 세면 이미지 참조만 이어 붙인
 * 본문 하나가 예산을 「정확히 지킨 채」 다른 씨앗의 **네 배**를 낸다 — 이미지는 `<img>` 와 그
 * 주소·대체문구를, 강조는 태그 한 쌍을 낸다. 산출이 다른데 같은 몫을 빼 가면 예산이 뜻을 잃는다.
 *
 * 가중치는 산출 크기 **순서**를 따른다 — 비율까지 맞지는 않는다. 재는 법과 실측값은
 * `node --experimental-strip-types --test src/lib/markdownRender.test.ts` 의
 * 「🔴 글자 노드는 구조 비용이 0 인데 링크와 같은 몫을 문다」가 그대로 담고 있다:
 * 구조 비용은 이미지 85 · 코드 53 · 링크 35 · 강조 17 · 기울임 9 바이트라 «가장 싼 것 = 1» 로
 * 정규화하면 1 / 1.9 / 3.9 / 5.9 / 9.4 인데 값은 1 / 1 / 2 / 2 / 3 이다. 순서만 맞춘 **거친**
 * 값이고, `text` 는 아예 역전돼 있다(아래 주석).
 * 가중치가 실제로 걸리는지는
 * `node --experimental-strip-types --test src/lib/markdown.test.ts` 의 「비싼 인라인일수록 적게
 * 받는다」가 잰다(이미지 < 링크 < 강조).
 */
export const INLINE_COST: Record<Inline["kind"], number> = {
    image: 3,
    code: 2,
    link: 2,
    // 🔴 **이 값은 산출 대비 과하고, 그 대가가 실재한다.** 글자 노드만 있는 씨앗을 만들 수 없어서
    //    (글자는 다른 노드 «사이» 에만 생긴다) 안전한 쪽으로 잡은 값인데, 실측하면 `text` 는
    //    렌더러가 글자 노드를 **요소로 안 감싸** 구조 비용이 **0** 이다 — 링크(35B)·코드(53B)와
    //    같은 몫 2 를 무는 셈이다. 그런데 정상 기사의 몫은 **78% 가 이 `text`** 다. 재현:
    //    `node --experimental-strip-types --test src/lib/markdownRender.test.ts`
    //    (「🔴 글자 노드는 구조 비용이 0 인데 링크와 같은 몫을 문다」가 그것을 재고 못박는다)
    //    ⛔ 「산문은 블록 예산이 먼저 걸리므로 이 값 때문에 잘리는 글은 없다」고 적혀 있었는데
    //       **거짓이다** — 세 축 중 몫이 먼저 닿는다. 그 문장이 근거로 든 시험(「예산 안의 장문
    //       기사」)은 블록·잎만 재고 이 축을 안 잰다. 확인:
    //       `node --experimental-strip-types --test src/lib/markdown.test.ts`
    //       (「🔴 세 축 중 몫이 먼저 닿는다」)
    //    이 값을 1 로 내리면 첫 손실이 **24.4KiB → 39.8KiB** 로 넓어진다(재현: 아래 예산 KDoc 의
    //    씨앗에서 절 수를 올려 가며 서식 손실이 처음 나는 자리를 본다).
    //    ⚠ 공짜는 아니다 — 몫이 싸지면 그만큼 마크업 노드가 더 들어와 **최악 산출도 는다**.
    //       얼마나 느는지는 **씨앗에 따라 다르다**(한 갈래만 채운 씨앗에서는 0, 섞인 씨앗에서는
    //       두 자릿수 %). 결정 전에 **자기 사이트의 글 분포로 재라** — 여기에 한 숫자를 적으면
    //       그 씨앗의 답이 전체의 답으로 읽힌다.
    //    **고치려면 제품 결정이 필요하다** — 이 값이 바뀌면 지금 잘리던 글이 안 잘린다.
    //    시험 둘이 이 값을 못박고 있으니 함께 움직여야 한다.
    text: 2,
    strong: 1,
    em: 1,
};
 ...
 *
 * ⚠ **판정이 여기 있어야 한다.** 개수로 자르고 밖에서 가중치를 세면, 한 블록짜리 문서는
 *   첫 호출에 남은 몫을 통째로 받아 **가중치가 한 번도 안 걸린다**.
 */
 ...
 *
 * ## ⚠ 이 값들이 묶는 것은 «증폭» 이지 동시 렌더 수가 아니다
 *
 * 상한은 문서 **하나**의 산출을 일정 크기 아래로 묶는다. 그 크기가 어느 사이트에서 동시 렌더
 * 몇 개를 견디는지는 **여기서 말할 수 없다** — 사이트 메모리 상한과 서빙 형상에 달려 있다.
 *
 * ⛔ **「K 개까지 산다」는 문장을 여기에 적지 마라** — 같은 주소를 K번 쳐도 ISR 이 한 번만
 *    그리므로 그런 씨앗은 동시성을 재지 않는다. 재려면
 *    **캐시 키가 다른 주소**(서로 다른 슬러그)로 치고 실제로 겹쳤는지를 증명해야 한다.
 *
 * ⚠ 셋 중 **잎과 인라인이 비용을 지배한다** — 그 둘을 고정한 채 블록만 두 배로 해도 최악 산출은
 *   거의 안 는다. 그래서 블록은 장문 기사가 여유를 갖도록 잡고, 잎·인라인을 더 좁게 잡았다.
 *
 * ## ⚠ 정상 장문 글의 여유는 축마다 다르다 — **몫이 먼저 닿는다**
 *
 * 아래 수치는 **실측**이고, 시험이 그대로 단언한다. 재현:
 * `node --experimental-strip-types --test src/lib/markdown.test.ts`
 * (「🔴 세 축 중 몫이 먼저 닿는다」 — 수치가 바뀌면 그 시험이 red 다)
 *
 * 절 50개 · 문단 300개(문단마다 링크 1 · 강조 1) · 표 10개 · 목록 17개짜리 기사(본문 20KB):
 * 블록 377/1,000 · 잎 942/2,000 · **몫 4,084/5,000**. 블록·잎은 절반을 안 쓰는데 **몫은 82%** 다.
 *
 * 🔴 **그래서 세 축 중 몫이 먼저 닿는다.** 같은 밀도로 절 60개(본문 23KB)까지는 서식이 그대로
 * 서고, 절 70개(본문 27KB)에서 문단 54개가 서식을 잃는다(글자는 남는다). 「병적인 문서만
 * 잘린다」가 **아니다** — 본문 크기 상한(그것은 API 가 정한다)보다 훨씬 앞에서 이 축이 닿는다.
 *
 * ⚠ **그 씨앗은 밀도가 아주 높다 — 「27KB 기사가 잘린다」로 읽지 마라.** 문단 하나가 45바이트인데
 *   그 안에 링크 1 · 강조 1 이 들어 있다. 실제 기사 문단(한국어 산문 340바이트 남짓)으로 다시
 *   재면 첫 손실은 **141~291KiB** 다 — 링크가 문단마다 있고 표·목록까지 섞이면 141KiB, 산문만이면
 *   291KiB(그때는 몫이 아니라 블록 예산이 먼저 문다). 방향은 그대로다: 본문 상한보다 앞에서 닿는다.
 *   링크·강조가 촘촘한 장문 기사를 싣는 사이트라면 이 상수를 먼저 보라.
 *
 * ⛔ **이 값을 올리려면 사이트 메모리 상한부터 확인하라.** 「좀 넉넉하게」로 올리면 그만큼이 그대로
 *    한 요청의 메모리가 되고, 겹치는 요청이 그 사이트를 죽인다. 재는 법은 배송 형상
 *    (`.next/standalone` 산출물(`node server.js`))을 메모리 상한 아래 띄우고 **서로 다른 슬러그**를
 *    동시에 치는 것이다.
 ...
 *   `&amp;` 다섯 자) — 렌더러까지만 봐도 **약 5배**이고 쪽 전체는 RSC 사본이 더 붙는다.
 *   그 형상은 예산 셋을 거의 안 건드린다(블록 1 · 몫 2) — **구조 예산으로는 못 막는 축**이다.
 *   재현: `node --experimental-strip-types --test src/lib/markdownRender.test.ts`
 *   (「이스케이프 증폭 — 예산이 못 막는 축이 있다」)
 *
 * 왜 증폭이 위험한가: **한 겹의 렌더가 그만큼 메모리를 잡고, 그 시간만큼 프로세스를 붙든다.**
 * 그 주소는 공개라 비용을 익명 방문자가 반복해서 유발하고, 서빙 프로세스에는 테넌트가 함께 산다.
 *
 * ⛔ 「산출이 커지면 ISR 캐시가 항목을 **거부해** 요청마다 다시 그린다」고 적혀 있었는데,
 *    이 팩이 도달할 수 있는 크기에서는 그런 거부를 못 봤다. 근거로 쓰지 마라 — 쓰려면
 *    자기 사이트를 띄우고 같은 쪽을 두 번 쳐서 상류 왕복이 느는지 직접 보라.
 *
 ...
 * → `블록 1 인라인 2501` (`` `x` `` 는 `code` 라 몫 2 — 5,000/2 = 2,500 + 꼬리 글자 1)
 *
...
export function parseMarkdown(source: string): Block[] {
    ...
    while (i < lines.length) {
        ...
        //    서빙 프로세스가 힙에서 죽는다 — 그 글은 공개 URL 이라 익명 방문자가 그 비용을
        //    반복해서 유발할 수 있고, 서빙 프로세스에는 테넌트가 함께 산다.
