 ...
 *
 * ## ⚠ 이 값들은 **사이트 메모리 상한에서 역산했다**
 *
 * 기준은 「예산을 **지킨** 최악 문서를 동시에 여럿이 쳐도 서빙 프로세스가 산다」이다. 값을 네 배로
 * 하면 동시 요청 한 겹은 살고 **두 겹에서 죽는다** — 렌더 한 쪽의 봉우리가 그만큼 커지기 때문이다.
 * 숫자는 사이트 상한마다 다르니 **자기 상한에서 다시 재라**(재는 법은 아래).
 *
 ...
 * 재현: `markdown.test.ts` 의 「🔴 세 축 중 몫이 먼저 닿는다」.
 *
 * ⛔ **이 값을 올리려면 사이트 메모리 상한부터 확인하라.** 「좀 넉넉하게」로 올리면 그만큼이 그대로
 *    한 요청의 메모리가 되고, 두 번째 요청이 그 사이트를 죽인다. 재는 법은 배송 형상
