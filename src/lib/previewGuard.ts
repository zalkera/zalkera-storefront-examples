/**
 * 미리보기 모드 쓰기 차단의 **판정**. 순수 함수라 전수로 시험한다(`previewGuard.test.ts`).
...
/**
 * 미리보기에서도 막지 **않는** 경로. 사유는 각 라우트 파일 머리의
 * `// zalkera-allow-preview-write: <이유>` 마커에 한 줄로 있다.
 *
 * 사유는 **보이는 글자**여야 한다 — 빈 사유는 물론이고 폭 없는 공백·결합 문자만 적은 것도
 * 면제가 안 된다(`previewGuard.test.ts` 가 판정한다). 다만 한글 채움문자(U+3164)처럼 문자
 * 범주로는 글자인 것까지 막지는 못한다 — 마커는 사람이 읽는 장치이지 봉인이 아니다.
 *
 * ⚠ 여기 넣는 것은 "운영 데이터를 써도 좋다"는 뜻이다. 넣기 전에 그 라우트가 **누구의** 데이터를
 *   건드리는지 확인하라 — 로그인·로그아웃은 미리보기어 자기 세션이고, `revalidate` 는 고객 데이터가
...
/**
 * 이 요청을 미리보기에서 막아야 하는가. **미리보기 여부는 여기서 안 본다** — 부르는 쪽이 판단한다
