 ...
 *   백엔드 카트도 같은 규칙으로 마지막 광고 접점을 남긴다(담기마다 넘긴다).
 ...
 * 인코딩한 쿠키 값의 상한.
 *
 * 브라우저가 말없이 버리는 4KB 가 상한선이지만 **그 한 칸만 보고 정하면 안 된다** — 이 쿠키는 `path: "/"` 라
 * 정적 청크·폰트·`/media/{id}` 까지 **모든 오리진 요청**에 실리고, 같은 경로에 이미 액세스·리프레시 토큰
 * (각 1.5~3 KB)과 장바구니·로그인 표식이 있다. 합이 Node 기본 헤더 상한(16 KB)에 다가가면 431 이고,
 * 안 넘어도 로그인 고객의 모든 에셋 요청이 그만큼 무거워진다(심의 성능 축).
 *
 * [PRIORITY] 가 캠페인·소스·매체를 앞에 두어 넘치는 쪽부터 우아하게 깎이므로, 보통 크기의 유입은 **매출을 세는 세 칸이
 * 그대로 남는다**(한글 캠페인명은 인코딩하면 세 배 넘게 는다).
 *
 * ⚠ **값 한 칸의 상한([VALUE_MAX] 255자)과 다른 층이다.** 한글 255자짜리 캠페인명은 인코딩하면 2,295자라
 * 이 예산에 안 들어간다 — 그때 쿠키는 그 칸을 **안 싣는다**(리드 폼이 그 페이지 쿼리에서 직접 잡는 경로는 그대로다).
 * 그런 값을 30일 동안 모든 요청에 지고 다니는 것보다 낫다는 판단이다.
 */
const COOKIE_VALUE_BUDGET = 1024;
