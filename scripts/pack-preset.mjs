...
const SOURCE_EXCLUDES = [
    ...
    "src/",
    "dist-presets/",
    // ⚠ **서드파티에게 손으로 건네는 명세다 — 배송물이 아니다.** 팩을 받은 고객에게는 `AGENTS.md` 가
    // 그 자리이고, 두 벌이 트리에 있으면 어느 쪽이 이 레포의 규약인지 갈린다. 그리고
    // `spec/handoff/` 는 **특정 납품건의 결함 보고서**라 전 테넌트에 복제될 물건이 아니다.
    // 재현: 이 줄을 지우고 굽고 `unzip -l dist-presets/skeleton-<판>.zip | grep spec/` → 3건
    "spec/",
    ...
    // 스위트가 그 실패를 **자기 결함으로** 읽는다 — 팩 4벌이 자기 검수에서 전부 반려됐다.
