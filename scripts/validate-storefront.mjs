 ...
 *  · **거짓 양성 가능성** — 상용 서빙 중인 두 사이트(bix·credium)가 **둘 다 걸린다**(실측 2/2).
 *    자기 방식으로 막은 라우트도 우리 심볼을 안 쓰면 걸린다는 뜻이다.
 *
 ...
 *    **그 사이트는 실제로 안 막고 있었다.**
 *
 *    그래도 이 축을 관문에서 빼는 판단은 유지한다 — 근거가 "다른 수단으로 막고 있다"가 아니라
 *    **"이 검사가 안전을 재지 못한다"** 이기 때문이다(위 거짓 음성). 통과해도 안전하지 않은 검사를
 *    관문에 놓으면 안전하다는 착각만 판다. 다만 **막지 않는 대신 반드시 보여야 한다** —
 *    업로드·검수 결과에 이 경고를 띄우는 것이 이 결정의 나머지 절반이고, 그것 없이는 방치다.
 *
