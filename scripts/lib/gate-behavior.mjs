#!/usr/bin/env node
/**
 * 미리보기 관문이 **실제로 막는가** — 문면도 매니페스트도 아니고 **띄워서** 잰다.
 *
 * 등재 검사(`gate-probe.mjs`)는 "관문이 실렸는가·무엇을 덮는가"만 본다. 그것으로는
 * ⑴ 판정을 무력화한 관문(`if (true) return next()`)과 ⑵ 라우트 가드까지 함께 제거된 소스를
 * 못 가른다. 그 둘이 겹치면 미리보기 쓰기 보호가 0 인데 전 검사가 초록이 된다.
 *
 * 그래서 미리보기 빌드를 기동해 실 HTTP 로 묻는다.
 *
 * ## 오라클
 *
 ...
 *   "존재하는 경로"가 되고, 그 핸들러가 미리보기에서 같은 403·코드를 내면 관문이 없어도 통과한다.
 *   `[slug]/page.tsx` 를 `[slug]/route.ts` 로 바꾸면 페이지 프로브도 같다.
 *   재현: 관문을 무력화하고 `src/app/api/[...gate]/route.ts` 에 미리보기일 때만 403 을 내는 핸들러를 두면 rc 0.
 ...
 *
 * 미리보기 쪽은 **상태와 코드를 둘 다** 요구한다(`403` + `PREVIEW_READ_ONLY`). 코드만 보면
 * `200 {"code":"PREVIEW_READ_ONLY"}` 만 내는 사칭 서버가 통과한다.
 *
 * **음성 통제군**: 미리보기가 아닌 빌드에서 **관문이 돈 흔적이 없어야** 한다. 프로브의 4xx·5xx 는
 * 정상이다 — 그 경로는 존재하지 않으므로 404 가 맞고, `[slug]` 가 받는 페이지 프로브는 콘텐츠가
 * 없어 500 이 난다. "서버가 통째로 죽었는가"는 읽기 한 건(`GET /` 200)이 따로 진다.
 *
 * 미리보기 여부는 **빌드와 런타임 양쪽**에 같게 준다. 판별자가 받는 이름이 둘이라(`PREVIEW_ENV_NAMES`)
 * 한쪽만 지우면 미리보기가 아닌 빌드가 실행 시점에 미리보기로 돌아 통제군이 전부 걸린다.
 *
 * ## 포트는 잡아서 쓴다
 *
 ...
 *   그 루트에서 미리보기 빌드를 한 번, 미리보기가 아닌 빌드를 한 번 굽고 각각 standalone 을 띄운다.
 */
...
/**
 * 미리보기 판별자가 받는 이름 **전부**. `src/lib/preview.ts` 와 같이 움직여야 한다 —
 * 여기 하나를 빠뜨리면 미리보기가 아닌 빌드가 미리보기가 되어 거짓 반려가 난다.
 * 첫 번째가 정본 이름이다(미리보기 빌드에 그것을 준다).
 */
const PREVIEW_ENV_NAMES = ["NEXT_PUBLIC_ZALKERA_PREVIEW", "NEXT_PUBLIC_ONEQUE_PREVIEW"];

/**
 * 자식의 **판정을 바꾸는** 상속 변수. 미리보기 이름과 같은 부류다 — 값이 아니라 뜻이 바뀐다.
...
function build(preview) {
    ...
    //   도 수용한다 — 하나만 지우면 환경에 구 이름이 있을 때 "미리보기가 아닌" 빌드가 실제로는 미리보기가 되고,
    ...
    if (r.status !== 0) {
        console.error(`[gate-behavior] ${preview ? "미리보기" : "미리보기가 아닌"} 빌드 실패 — 판정할 수 없습니다.`);
...
async function serve(preview, fn) {
    ...
    //   구 이름이 남아 있으면 미리보기가 아닌 빌드가 실행 시점에 미리보기로 돌아 음성 통제군이 전부 걸린다.
...
async function probe(base) {
    ...
    // 읽기 통제군. 미리보기가 아닌가 통째로 죽은 상태를 "0개 차단"으로 읽지 않기 위한 자리다.
    let home = 0;
    try {
...
// 하나뿐이다. 상태 전체를 위반으로 치면 정상 팩이 거짓 반려된다 — 미리보기가 아닌 서버는 그 경로들에
// 4xx 를 낸다(존재하지 않는 경로이므로 정답이다).
// 재현: 미리보기가 아닌로 굽고 띄운 뒤 직접 물어라 — 통과 줄에는 프로브 상태가 안 실린다.
...
if (bad.length) {
    console.error("[gate-behavior] **미리보기에서 쓰기가 안 막힙니다** — 관문이 그 경로에 안 걸립니다:");
    for (const r of bad) console.error(`  ${r.path}  → ${r.status} ${r.code || "(code 없음)"}`);
    console.error("  matcher 가 그 형태를 빼고 있거나, 관문 판정이 무력화됐습니다.");
    process.exit(1);
}
if (on.home >= 400) {
    console.error(`[gate-behavior] 미리보기에서 읽기까지 막힙니다 — GET / → ${on.home}. 관문이 읽기를 먹고 있습니다.`);
    process.exit(1);
}
if (falsePositive.length || off.home >= 400) {
    console.error("[gate-behavior] 미리보기가 아닌데 막힙니다 — 운영 서빙이 막힙니다:");
