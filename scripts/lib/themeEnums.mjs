 ...
 *
 * ⚠ **정규식으로 읽지 않는다 — 파서에게 시킨다.** 이 선언은 정규식이 못 읽는 것을 둘 담고 있다:
 *   ⑴ 값 안의 **템플릿 보간**(`` `…${SYSTEM_STACK}` ``) — 중괄호 범위를 `[^}]*` 로 잡으면 그 첫 `}` 에서
 *      잘려 뒤 항목이 통째로 안 보인다.
 *   ⑵ **따옴표 키**(`"noto-serif-kr":`) — `(\w+)\s*:` 는 못 잡고, 반대로 값 안의 `https:` 같은 것을
 *      키로 잡는다. 못 잡으면 정상 시드를 거짓 반려하고, 잘못 잡으면 계약 밖 값을 통과시킨다.
 *   파서는 문자열·주석 안의 가짜 선언에도 안 속는다.
 *
