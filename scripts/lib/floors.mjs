...
export const REQUIRED_FLOORS = {
    ...
    "src/lib/markdown.test.ts": 47,
    "src/lib/mediaRef.test.ts": 8,
    "src/lib/markdownRender.test.ts": 8,
...
/**
 * 게이트가 `--judgment=<파일>` 로 남긴 판정(`{skipped, reduced}`)을 **사람이 읽는 문장**으로. `verify-zip` 이
 * ✅ 줄에 싣는다. 판정 자체(rc·하한)는 게이트가 이미 집행했다 — 여기는 보고 문면뿐이다.
 *
 * 형태가 아니면 빈 목록 — 완화가 없었던 것으로 **말하는** 것이 아니라(그 사실은 게이트 출력이 든다),
 * 옛 게이트가 파일을 안 만든 경우를 반려로 만들지 않기 위해서다.
 *
 * @param judgment 파싱된 판정 객체(또는 못 읽었으면 `null`)
 */
