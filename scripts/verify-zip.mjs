 ...
 * 납품 zip 검수 러너.
 *
 * zip 하나를 받아 **받아도 되는 물건인지** 기계로 판정한다. 라이선스 출처와 행위 안전은 기계가
 * 못 재므로 통과해도 인수 확정이 아니다 — 사람이 볼 것은 통과 메시지가 열거한다.
 *
 * **판정을 복사하지 않는다.** 규약 검사는 `validate-storefront.mjs` 를 srcDir 인자로 **참조 실행**
 * 한다. 그 파일은 `@zalkera/client` 의 `zalkera-validate` 를 부르는 얇은 wrapper 라, 참조 실행하는
 * 순간 판정이 **설치본 한 벌**에서 나온다. 사본을 뜨면 그 순간 드리프트가 시작된다.
 *
 * ⚠ **사본을 손으로 고치지 마라.** 이 러너는 다른 자리에도 바이트 사본으로 나간다. 사본 쪽만
 * 고치면 두 벌이 서로 다른 것을 재게 되고, 어느 쪽이 참인지 알 방법이 없다. 고칠 곳은 이 파일이고
 * 사본은 다시 복사해서 맞춘다.
 *
 * ── 이 러너가 왜 빌드까지 도는가 ───────────────────────────────────────
 * **빌드 성공과 서빙 요건은 서로 다른 것을 잰다.** `npm run build` 가 exit 0 이어도 서빙 박스가
 * 요구하는 `.next/standalone/server.js` 가 안 나올 수 있고, 그러면 그 박스에서 반려된다(exit 4).
 * 빌드 종료코드만 보면 "검사 통과"가 "서빙된다"를 뜻하지 않는다. 그 간극을 ⑧ 산출물 검사가 닫는다 —
 * **이미 지불한 빌드에서 옳은 것을 읽는다**(추가 비용 0).
 *
 ...
 * 종류의 거짓이다.
 *
 * ⚠ **`--ignore-scripts` 를 격리 수단으로 읽지 마라.** 이 러너는 그 뒤에 `npm run build`·
 *   `npm run check:aeo` 를 돌리고 `--pack` 에서는 빌드를 두 번 더 굽는다 — 즉 **zip 의 스크립트를
 *   검수자 기계에서 실행한다.** 빌드 없이 서빙 요건을 잴 방법이 없어 그렇게 설계했다.
 *   (하한 집행은 `npm test` 가 아니라 `node --test` 를 직접 부르므로, zip 이 정한 `test` 스크립트
 *   문자열은 안 돈다 — 임의 명령 싱크 하나가 줄었을 뿐 격리는 아니다.)
 ...
 *
 * ⚠ 이 러너는 바이트 사본으로 나간다. 단일 파일이 아니라 `verify-zip.mjs` + `lib/` 를
...
반려 — 위 ❌ 항목을 고쳐 재납품 요청하십시오.`);
    ...
    if (unzip.status !== 0) {
        ...
        // ⚠ **X 축(교차사이트 위조 가드)은 `--pack` 에서 반려 사유다.**
        //
        //   검사기 쪽에서 X 를 error 로 올리는 것은 **영구 금지**이고 그 논거는 옳다 — X1 은 "교차
        //   오리진을 막았는가"가 아니라 "우리 심볼을 썼는가"를 재므로, 통과가 안전을 뜻하지 않고
        //   **재지 못하는 것으로 남의 빌드를 막으면** 대가만 치른다. 그것은 **고객 빌드**의 이야기다.
        //
        //   여기는 다르다. `--pack` 은 "이 zip 이 본사 카탈로그에 올라간다"는 선언이고, 팩의 결함은
        //   **그 팩으로 만든 모든 테넌트 사이트에 복제**되며 회수가 안 된다. 우리 물건을 우리 기준으로
        //   더 높게 잡는 것은 고객에게 부담을 지우는 일이 아니다.
        //
        //   검사기 주석이 스스로 적은 "막지 않는 대신 **반드시 보여야 한다**"의 나머지 절반이기도 하다 —
        ...
        if (packMode && xWarnings.length) {
            record(
                "교차사이트 위조 가드(팩 기준)",
                false,
                `\n   ${xWarnings.length}건 — 팩은 전 테넌트에 복제되므로 이 축은 반려 사유입니다.` +
                    `\n   ${xWarnings.slice(0, 8).join("\n   ")}`,
            );
            failed = true;
        }
        ...
        if (hasPkg && lock && !failed) {
            ...
            if (!run("npm ci", "npm", ["ci", "--ignore-scripts", "--include=dev", "--no-audit", "--no-fund"])) failed = true;
                ...
                if (!run("npm run build", "npm", ["run", "build"])) {
                    ...
                    //     예약 세그먼트 그림자화). 소스 배선 · `srcRoute` · `status` 세 축이 각각 맡는다.
                    ...
                    //     `--pack` 에서만 돈다(미리보기 빌드와 일반 빌드를 각각 한 번 더 굽는다).
                    //
...
if (xFindings.length > 0) {
    console.log(
        packMode
            ? "  · ⚠️ [X1~X3] 교차사이트 위조 가드 — `--pack` 에서는 **반려 사유**다(팩은 전 테넌트에 복제된다)"
            : "  · ⚠️ [X1~X3] 교차사이트 위조 가드 경고 — 이 모드는 안 막는다(`--pack` 에서는 반려한다)",
    );
}
console.log("  · 실제 개시 후 화면 확인 + `globals.css` 의 `@theme` 토큰을 1회 고쳐 반영 확인");
