...
## 7. 색은 소스가 정본입니다 (정직 고지)

색·타이포의 값은 **소스**에 있습니다 — 우리 시작 팩에서는 `src/app/globals.css` 의 `@theme`
토큰 한 자리입니다. 콘솔에는 색 설정이 없습니다.

당신이 프론트를 직접 만든다면 그 자리도 당신 것입니다 — 토큰을 쓰든 CSS 를 직접 쓰든 자유이고,
잘커라가 그 값을 덮어쓰지 않습니다. 원천이 하나이므로 「콘솔에서 바꿨는데 화면이 그대로」 같은
일이 생기지 않습니다.

⚠ 대신 색을 바꾸면 **다시 빌드해야** 반영됩니다. 우리가 서빙하는 사이트는 소스를 올리면
자동으로 다시 빌드합니다(실측 30초 안팎).
## 셋업 요약
