...
그 안의 시험 몇은 §2-1 ⑴ 이 지우는 디렉터리를 읽습니다 — `astGuards.test.ts` 의 넷(`src/app/blog/`·
`src/components/sections/`)과 `blogListRender.test.ts` 의 다섯(`src/app/blog/page/[n]/page.tsx`). 그 디렉터리가 없으면
