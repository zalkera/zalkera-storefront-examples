...
| | 어디서 오나 | 무엇 |
| --- | --- | --- |
| **얼굴** | **시안** | `src/app/(landing)/page.tsx` · `(landing)/landing.css` · `(landing)/layout.tsx` 의 `metadata`·`viewport`(시안 `<head>` 의 값) · `public/` 의 이미지·폰트 |
...
화면은 시안 그대로입니다. SEO·AEO 로 손대는 자리는 **`<head>` 메타와 JSON-LD** 둘뿐이고, 거기 들어가는 값도
**시안에 있던 문구**이거나 **콘솔에 입력된 사업자 정보**입니다. 지어내는 값은 없습니다. 플랫폼의 AEO 보장표
...
(또는 `LocalBusiness`) JSON-LD 를 요구합니다. 판정 지점은 소스가 아니라 **개시된 산출물**입니다.

**⑴ `<head>` → `src/app/(landing)/layout.tsx` 의 정적 `metadata`·`viewport`** — 시안 `<head>` 에서 그대로 옮깁니다.
...
씁니다. 값은 콘솔의 사이트 설정(회사명·전화·이메일·주소)에서 오고 비어 있으면 필드가 빠집니다.
...
export const revalidate = 600;                                  // 시작 팩 홈과 같은 ISR — 콘솔에서 고치면 10분 안에 반영
...
- `organizationJsonLd` 는 `@type` 을 콘솔의 업종으로 고르고 `url` 을 절대경로(`siteUrl()`)로 냅니다 —
...
- **만들지 않는 것**(보장표의 의도적 부정 보장): 시안의 후기·별점·처리 건수는 시뮬레이션입니다 —
  `Review`·`AggregateRating`·`Product`·`Offer` 를 내지 않습니다. 카카오 오픈채팅 주소는 `sameAs` 가
  아닙니다(소셜 프로필이 아닙니다). 주소·이메일을 시안에서 지어 채우지 않습니다(콘솔 값만).
...
개발 서버가 뜬 상태에서 `<head>` 와 홈 그래프를 봅니다(§2-2 의 SEO·AEO 절).
...
# ✅ siteWide/robots · ✅ siteWide/sitemap · ✅ siteWide/absolute-urls · ⏭️ sitemap-covers-required-routes(무주장이라 SKIPPED) → rc 0
```
...
가 나옵니다(§2-2 ⑵ — 간판은 못 겁니다). 그 출력에서 볼 것은 `required/home-organization` 이 PASS 인지,
