...
#### ⚠ 스크린샷을 찍기 전에 **폰트가 다 뜨기를 기다리십시오**

안 기다리면 아직 대체 글꼴로 그려진 화면이 찍힙니다. 글자가 있는 자리가 **전부** 어긋나므로
문서 전체가 다르게 나오고, 원인을 팩에서 찾게 됩니다.

```js
await page.evaluate(() => document.fonts.ready);   // 그다음에 screenshot
```

실측(같은 팩을 두 방식으로 재서 원본과 대조):

| 촬영 방식 | 원본과 다른 픽셀 |
| --- | --- |
| 기다리지 않음 | **5.212%** (358,218 / 6,872,320) |
| `document.fonts.ready` 뒤 | **0.053%** (3,642 / 6,872,320) |

같은 페이지를 같은 방식으로 두 번 찍으면 **0.000%** 다 — 즉 위 5.2% 는 촬영 잡음이 아니라
**측정 방식이 만든 가짜 차이**였다. 남는 0.05% 는 애니메이션 프레임 타이밍이다.

폰트를 `data:` 로 인라인한 사본과 파일로 서빙하는 팩을 견줄 때 특히 크게 벌어진다 —
전자는 즉시 뜨고 후자는 요청이 따로 나가기 때문이다.
