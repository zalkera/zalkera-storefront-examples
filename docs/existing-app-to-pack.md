...
헤더를 내면 앱 값이 그대로 나갑니다. (이 셋 밖에서 서빙이 손대는 헤더는 따로 있습니다 — HTML 문서 응답의
`cache-control` 은 Next 의 장기 캐시 값을 즉시 재검증으로 바꾸고, 플랫폼 주소(`*.zalkera.com`)로 볼 때는
`X-Robots-Tag: noindex, nofollow` 가 붙습니다. 이 절은 아래 셋만 다룹니다.)
...
- 그 구형 브라우저에서까지 꼭 열어야 한다면 `X-Frame-Options` 를 **그 이름으로** 다른 값으로 내십시오 — 그러면
  우리 기본값은 물러섭니다(그것은 서빙이 보장합니다). 다만 **그 브라우저가 그 값을 어떻게 해석하는지는 우리가 재지
  않았습니다**. IE11 은 `ALLOW-FROM https://partner.example` 형식을 문서상 받습니다(현행 브라우저는 그 값을 무시하고
  `frame-ancestors` 를 따릅니다). 표준에 없는 값을 무시하는 브라우저에서는 **누구나** 프레임에 넣을 수 있게 되니,
  이 길은 그 브라우저에서 실제로 확인한 뒤에만 쓰십시오.
...
### 3-2. 문안 대조 — `zalkera-port-text`

🔴 **기준선은 이관 «전에», 원본 라이브에서 뜹니다.** 커스텀 도메인이 붙는 순간 원본은 사라지고,
그 뒤에는 무엇과 견줄지가 없어집니다. 원본이 이미 죽었으면 조사 때 떠 둔 HTML(`archive`)이나 원본
소스를 그 판으로 빌드한 것(`source-build`)으로 내려가고, **그것도 없으면 「기준선 없음」으로 적습니다** —
이관본에서 뜬 것을 원문이라 부르지 마십시오.

```bash
# ① 이관 전에 — 원본에서
npx -y -p @zalkera/client -p parse5 zalkera-port-text \
    capture https://원본.example port-text --source live

# ② 이관본 — §3-1 로 구운 뒤 «조립해서» 띄운 산출물에 댑니다(standalone 만 띄우면 거짓 404)
cp -R .next/standalone/. out/ && cp -R .next/static out/.next/static && cp -R public out/public
PORT=3400 node out/server.js &
npx -y -p @zalkera/client -p parse5 zalkera-port-text \
    check port-text http://127.0.0.1:3400 --rules port-text/rules.json --report port-text/report.json
```

`exit 0` 이어야 넘어갑니다. `1` 은 어긋남, `2` 는 **실행 불가**(인자·네트워크·`parse5` 부재)이며
...
1. **원본 레포가 라이브보다 뒤처져 있다** — 가장 흔합니다. 먼저 의심하십시오.
2. **조건부 렌더가 env 때문에 한쪽에서만 그려진다**(`{KEY && …}`) — **라이브와 같은 env 로 다시 구워**
   견줍니다.
3. **의도된 편차다** — 그러면 `port-text/rules.json` 에 **이유와 함께** 적습니다. 적지 않으면 다음
   사람이 그것을 결함으로 다시 쫓습니다.

```json
{
  "drop": [{"match": ".admin-link", "reason": "원본 앱의 관리자 링크 — 이관 대상 아님(의뢰서 §2)"}],
  "mask": [{"pattern": "\\d{4}-\\d{2}-\\d{2}", "token": "⟦DATE⟧", "reason": "게시판 작성일"}]
}
```

규칙은 기준선 파일 **밖**에 두고 `check` 시점에 **양쪽에 똑같이** 걸립니다. 한쪽만 적중하면 「선언된
편차」로, 양쪽 0건이면 「죽은 규칙」으로 찍힙니다. ⚠ **가린 비율에 상한이 있습니다**(기본 10% ·
`drop` 으로 지운 글자와 `mask` 로 가린 글자를 함께 셉니다) — 규칙으로 페이지를 통째로 가려 초록을
만들 수 없습니다.

⚠ **모바일/데스크톱 두 벌은 두 벌 그대로 둡니다.** 크롤러와 답변 엔진이 읽는 것이 DOM 텍스트이고
중복도 그 일부입니다. 한 벌로 합칠 거면 `drop` 규칙으로 **선언**하십시오 — 조용히 정리되는 일이
없어야 합니다.
...
- 검증 결과: `verify-zip --byo` rc · **`port-text/`**(`manifest.json` 의 `source` · `report.json` 의
  라우트별 판정 · 규칙과 이유) · 픽셀 차이(가린 영역 포함) · 관문 실측표
