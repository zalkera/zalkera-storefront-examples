...
| 소셜 로그인 교환을 **한 입구**(`exchangeSocialLogin`)로 | 싱글턴 `zalkera` 에서 `socialLogin` 을 뗐습니다 — 교환을 새로 짜도 state 대조를 빠뜨릴 수 없게. 대조·소각·교환의 행위를 시험이 잽니다 | `src/lib/zalkera.ts` · `src/lib/oauthState.ts` · `src/lib/session.ts` · `src/app/api/auth/social/route.ts` · `src/lib/oauthState.test.ts` |
