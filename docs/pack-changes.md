...
| 블로그·프리셋 섹션을 **지운 트리**에서 가드 하한이 미달로 반려되던 것 | 시안 레인(`docs/mockup-to-pack.md`)은 그 둘을 지우는데, 그 파일을 읽는 시험 아홉이 있어 지운 팩이 `verify-zip` 에서 막혔습니다. 대상이 없으면 그 시험은 이유를 찍고 건너뛰고 하한도 같은 수만큼 낮아집니다 | `src/lib/astGuards.test.ts` · `src/lib/blogListRender.test.ts` · `scripts/lib/floors.mjs` · `scripts/lib/floor-gate.mjs` · `scripts/lib/test-floors.json` · `scripts/lib/floors.test.mjs` · `scripts/lib/floorGate.test.mjs` |
