...
| `@zalkera/client` 0.41.0 | 교본에 **상담 신청(리드)의 문답 칸**과 **휴대폰 본인확인 시작**이 들어왔습니다 — 팩은 아직 부르지 않습니다. 인증을 붙일 때는 교본의 경고를 그대로 지키십시오: `available === false` 는 오류가 아니라 **그 단계를 건너뛰고 접수를 계속 받으라**는 뜻이고(막으면 광고비는 나가는데 리드가 0 이 됩니다), 표는 한 번만 먹습니다 | `npm install @zalkera/client@^0.41.0` · 루트 `llms.txt`(최신 zip 에서) |
