...
| 로그인 갱신 뒤 **`0.0.0.0` 으로 튕기던** 것 | 갱신 라우트가 서버가 뜬 주소로 이동을 만들었습니다 — 로그인 15분 뒤 새로고침·로그아웃 상태의 마이페이지에서 났습니다. 이동에 경로만 싣습니다 | `src/app/api/auth/refresh/route.ts` · `src/lib/redirect.ts` · `src/lib/redirect.test.ts` · `scripts/lib/test-floors.json` |
