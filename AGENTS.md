...
- 안 하면 무엇이 나빠지나 — **축마다 결과가 다르다.**
  ...
  - **게시글 조회수**(`recordPostView`): 레이트리밋이 아니라 **조회 dedup** 이다. 키가
    `sha256(방문자 IP|User-Agent)` 이고 **게시글별 · UTC 달력 하루** 단위다. 뭉치면 429 가 아니라
    **게시글마다 하루에 UA 종류 수만큼**으로 접힌다 — 실제의 한참 아래인데 **0 이 아니라서 눈치채기
    어렵다**("조회수가 1이 아니니 배선은 정상"이라는 자가진단이 여기서 틀린다).
