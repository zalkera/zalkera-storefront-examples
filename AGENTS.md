...
> **레포가 아직 없는 사람에게 줄 문서는 여기가 아니다.** 이 문서는 *이 레포를 고치는* 에이전트용이라
> 파일 좌표표가 중심이고 레포가 이미 있다는 것을 전제한다. 빈 디렉터리에서 새로 짜는 서드파티
> 개발자·AI 에게는 `spec/storefront-spec.md` 를 준다 — 그쪽은 자립 문서로 쓰여 있다.
