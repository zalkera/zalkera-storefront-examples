...
**범위를 정하는 자리는 `.prettierignore` 하나다.** `npm run format` 은 `prettier --write .` 이고, 무엇을 건너뛸지는 그 목록이 정한다. 그래서 손으로 돌리든 스크립트로 돌리든 **같은 범위**다 — 어느 쪽을 쓸지 고민할 필요가 없다.

무엇이 바뀔지 궁금하면 문서를 읽지 말고 **물어봐라**:

```bash
npx prettier --list-different .   # 아무것도 안 나오면 바꿀 것이 없다는 뜻이다
```

⚠ 이 절은 **두 판 연속 틀린 채로 배송됐다.** 처음엔 "CI 가 잰다"고 적었는데 CI 는 안 쟀고, 다음 판은 "루트 실행이 워크플로·문서까지 재포맷한다"고 적었는데 `.prettierignore` 가 이미 막고 있었다. 원인은 문장력이 아니라 **범위가 두 군데(스크립트 글롭 + 무시 목록)에 있었다는 것**이다 — 둘이 어긋나면 어느 쪽을 설명해도 거짓이 된다. 그래서 범위를 한 곳으로 모았고, 이제 이 절은 범위를 **서술하지 않는다.**
