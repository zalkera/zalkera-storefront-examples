...
이 값들은 임의로 고른 것이 아니라 **사이트 메모리 상한에서 역산한 것**이다. 올리면 그만큼이
그대로 한 요청의 메모리가 되고, 겹치는 요청이 그 사이트를 죽인다.

🔴 **셋 중 «몫» 이 먼저 닿는다 — 「병적인 문서만 잘린다」가 아니다.** 실측(절 50개 · 문단
300개(문단마다 링크 1 · 강조 1) · 표 10개 · 목록 17개 · 본문 20KB): 블록 377/1,000 ·
잎 942/2,000 · **몫 4,084/5,000 = 82%**. 같은 밀도로 **절 60개(23KB)까지는 서식이 그대로 서고,
절 70개(27KB)에서 문단 54개가 서식을 잃는다**(글자는 남는다). 링크·강조가 촘촘한 장문 기사를
싣는 사이트라면 **몫부터** 보라 — 본문 크기 상한보다 훨씬 앞에서 이 축이 닿는다.
재현: `node --experimental-strip-types --test src/lib/markdown.test.ts` 의
「🔴 세 축 중 몫이 먼저 닿는다」.
