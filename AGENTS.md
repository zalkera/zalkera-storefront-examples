...
- ⚠ **면제는 «읽기» 에만 붙는다 — 상태를 바꾸는 `GET` 을 새로 만들지 마라.** 위 근거는 *응답을 못
  읽는다*는 것이지 *요청이 안 간다*가 아니다. 교차 오리진에서 최상위 이동(`<a href>`·`window.open`
  ·`302` 리다이렉트) 한 번이면 세션 쿠키가 실린 요청이 그대로 도달하므로, 그 GET 이 무엇을 바꾸든
  바뀐다(`<img src>`·`<link>` 같은 서브리소스는 쿠키가 `sameSite: "lax"` 라 대개 쿠키 없이 간다 —
  그 차이에 기대지 마라. 판정이 쿠키에 안 걸린 GET 이면 그 벡터도 그대로 성립한다).
  **검사기는 이것을 아예 못 본다**(X1 은 변이 메서드만 재고 GET 은 안 센다) — 이 규칙을 지키는 것은
  사람과 이 문서뿐이다.
  기존 `GET /api/auth/refresh` 는 면제가 아니라 **개별 수용**이다: 피해가 자기 세션 강제 회전에
  그치고 응답이 곧 새 쿠키를 심는다는 근거로 받은 것이라(오너 결정), 다른 상태를 바꾸는 GET 은
...
- 광고 유입은 `src/middleware.ts` 가 방문자가 연 문서 요청의 캠페인·소스·매체·클릭 표지를 httpOnly 쿠키에 두고, 담기(`/api/cart/items`)·예약(`/api/booking`)·리드(`/api/lead`)
  ...
  🔴 **이 쿠키는 개인정보 처리방침의 고지 대상이다**(자동 수집 장치 · 30일 보관 · utm·클릭 표지 있음·랜딩 경로·referrer 호스트).
  이 팩에는 처리방침 **면이 없다**(`/policies` 는 반품·교환·배송 같은 **구매 정책**이다) — 사이트를 열기 전에
  처리방침 면을 만들고 이 쿠키와 상담 신청 수집 항목을 적어라. 안 적고 열면 고지 없이 추적하는 사이트가 된다.
...
> **검사기가 이것을 본다** — `npm run verify` 가 부르는 `@zalkera/client` 의 검사기가 `[I2]` 로 선언 누락을
> **경고**한다(`npm run validate` 로 따로도 돈다). 다만 **경고이고 파일 단위**다 — 같은 파일의 다른 호출이
> 이미 선언하면 그 파일은 통과하므로, 한 파일에 여러 호출이 있으면 이 절이 마지막 방어다.
> 규칙은 그대로다: **IP 민감 호출에는 `visitorIp()` 로 뽑은 `clientIp` 를 넘긴다.** 값을 헬퍼로 빼도 되고
> 조건부로 채워도 된다 — **출처가 `visitorIp()` 이면 된다.**
>
> ⚠ **검사기가 보는 것은 열 개이고, 교본이 더 요구하면 교본을 따른다.** `@zalkera/client` 0.41.0 의
> `startIdentityVerification` 은 교본이 `ctx.clientIp` 를 **필수**로 적는데 검사기 목록에는 없다 — 안 넘기면
> 본인확인 레이트리밋이 사이트 서버 IP 한 칸으로 뭉쳐 **그 사이트 방문자 전원이** 상시로 건너뛰기 경로를 탄다.
...
이 값들이 묶는 것은 **한 문서의 «증폭»** 이다 — 동시 렌더 몇 개를 견디는지는 사이트 메모리
상한과 서빙 형상에 달려 있어 여기서 말할 수 없다. 올리면 그만큼이 그대로 한 요청의 메모리가
되고, 겹치는 요청이 그 사이트를 죽인다.

⛔ **「K 개까지 산다」를 여기에 적지 마라** — 한 판 적혀 있었고 철회했다. 같은 주소를 K번 쳐도
ISR 이 한 번만 그리므로 그 씨앗은 동시성을 재지 않는다. 재려면 **캐시 키가 다른 주소**로 친다.

🔴 **셋 중 «몫» 이 먼저 닿는다 — 「병적인 문서만 잘린다」가 아니다.**
재현: `node --experimental-strip-types --test src/lib/markdown.test.ts` 의 「🔴 세 축 중 몫이
먼저 닿는다」가 아래 수치를 그대로 단언한다(실측).
절 50개 · 문단 300개(문단 45바이트에 링크 1 · 강조 1) · 표 10개 · 목록 17개 · 본문 20KB → 블록
377/1,000 · 잎 942/2,000 · **몫 4,084/5,000 = 82%**. 같은 밀도로 **절 60개(23KB)까지는 서식이
그대로 서고, 절 70개(27KB)에서 문단 54개가 서식을 잃는다**(글자는 남는다). 링크·강조가 촘촘한
장문 기사를 싣는 사이트라면 **몫부터** 보라 — 본문 크기 상한보다 훨씬 앞에서 이 축이 닿는다.
⚠ 그 씨앗은 밀도가 아주 높다. 실제 기사 문단(한국어 산문 340바이트 남짓)이면 첫 손실은
**141~291KiB** 다 — 「27KB 기사가 잘린다」로 읽지 마라.
...
⚠ **여기서 포맷 범위를 세지 마라.** prettier 는 `.prettierignore` 와 `.gitignore` 를 둘 다 보고, 무엇이 실제로 걸리는지는 도구가 판본마다 다르게 답한다. 범위를 알아야 하면 세지 말고 물어라:

```bash
npx prettier --list-different .
```

## 분석 태그 — env 로만, 발주처가 준 ID 만

`src/components/Analytics.tsx` 가 `NEXT_PUBLIC_GA4_ID` 를 읽어 값이 있을 때만 GA4 로더를 싣는다(루트 `layout.tsx` 가
렌더). 값이 없으면 `null` — **분석 태그**가 외부 호스트를 부르는 자리는 여기 하나이고, 그것도 ID 가 있을 때만 열린다
(소셜 로그인 리다이렉트는 별개다 — `src/lib/oauth.ts`).

- **콘솔 `analyticsConfig`(백엔드 site-config)를 여기로 잇지 마라.** 정적으로 서빙되는 페이지가 백엔드를 물게 된다.
  그 JSON 을 쓰려는 팩이 생기면 그 팩의 레이아웃에서 따로 잇고, 이 컴포넌트는 그대로 둔다.
- **ID 를 소스에 박지 마라.** 관리형은 콘솔 「사이트 환경변수」, 자체 배포는 `.env.local`. 소스에 박으면 팩을 받아 쓰는
  다음 사이트가 남의 속성에 이벤트를 쏜다.
...
보장표(잣대)는 **① `--guarantees <경로>` → ② 설치된 `@zalkera/client` 안의 운반본 → ③ 형제 체크아웃(`../backend`)의 정본** 순으로 찾는다. ②가 고객 기계의 기본 경로다 — 잣대를 npm 으로 배송하기 전에는 고객 zip 에서 이 검사기가 exit 2 로 죽어 있었다. 실행하면 어느 출처로 쟀는지 찍고, ②로 재는 자리에 ③도 있으면 둘을 대조해 갈라짐을 경고한다(운반본이 낡았다 = client 발행이 밀렸다). 잣대 해석만 확인하려면 `npm run check:aeo -- --print-guarantees`(크롤 없음·네트워크 불요) — `verify-zip.mjs` 가 zip 안에서 이걸 돌려 검사기 실행성을 기계로 센다. 종료코드 0=required 전부 통과 · 1=보장 미충족 · 2=실행 불가(인자·네트워크·표 부재). `planned`(표가 목표로 적어 뒀지만 아직 우리도 못 내는 표면)와 `conditional`(섹션을 썼을 때만 성립 — 크롤한 HTML 만으로는 "안 썼다"와 "썼는데 그래프가 없다"를 구분 못 한다)은 **실패로 세지 않는다**. 못 세는 것을 센 척하지 않는 것이 이 검사기의 규율이다.
