...
**`npm run format` 이 실패하면 네 편집이 원인이 아닐 수 있다.** 이 명령은 `src/` 밖도 보므로, 저장소 어딘가에 문법이 깨진 파일이 하나 있으면 명령 전체가 `rc 2` 로 선다(실측). 그때는 위 조회 명령으로 **어느 파일인지 먼저 확인해라** — 오류 줄이 파일명을 말해 준다.
