...
/**
 * 홈 — **`beauty-nail` 의 홈**. 이 팩의 소스는 이 팩의 것이다(오너 확정 2026-08-01 — 팩 4종은 소스를
 * 공유하지 않고 각자 온전한 소스를 갖는다. 종전의 "정본을 가리는 오버레이"는 개념째 없어졌다).
 *
 * 이 파일이 이 팩의 정체다. 저작물(선언 섹션)과 조회(직접 호출 진열)를 **한 화면에 나란히** 둔 것이
 * 사장이 LLM 에게 시킬 바로 그 일("여기에 시술 메뉴를 보여줘")의 본보기이고,
 ...
 * (`scripts/lib/wiring-parity.mjs` — presets/README "배선 동일성").
 *
...
export default async function Home() {
    ...
    return (
        <main className="py-8">
            ...
            <ProductRail title="시술 메뉴" moreHref="/products" moreLabel="메뉴 전체" />
