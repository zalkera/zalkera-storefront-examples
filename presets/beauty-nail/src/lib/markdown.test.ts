 ...
 * 태그 한 쌍을 낸다. 개수로 세면 이미지 참조만 이어 붙인 본문이 예산을 「정확히 지킨 채」
 * 강조 씨앗의 **네 배**를 낸다 — 작은 컨테이너를 죽일 수 있는 크기다.
 *
 ...
 * 증폭기다. 예산이 없으면 그 증폭이 본문 길이에 그대로 곱해져, 렌더 한 겹이 그만큼 메모리를
 * 잡고 그 시간만큼 프로세스를 붙든다. 그 주소는 공개라 익명 방문자가 반복해서 유발할 수 있다.
 *
 ...
 * ⚠ **표가 «유일한» 증폭 축이 아니다** — 제목이 칸보다 크다(위 블록 예산).
 * 그 글은 공개 URL 이라 렌더 한 겹의 메모리·시간을 익명 방문자가 반복해서 유발할 수 있다.
 *
 * ⛔ **「산출이 커지면 ISR 캐시가 항목을 거부해 요청마다 다시 그린다」를 근거로 쓰지 마라** —
 *    이 팩이 도달할 수 있는 크기에서 그런 거부는 관측되지 않았다(`markdown.ts` 의 같은 금지문). */
