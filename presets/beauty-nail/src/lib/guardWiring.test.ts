 ...
 * **5벌에서 일관되게** 지우면 `npm run validate` 가 rc 0 이고 경고조차 안 뜨는 자리가 있다(①층 일관 제거 ·
 * ②층 state 대조 통째 제거 · CORS 를 `next.config.ts`·`middleware.ts` 로 다는 경우).
 ...
 * 그것을 못 본다. X1 은 `assertSameOrigin` 만 세고, 순수 판정 함수 시험은 라우트를 안 지난다.
 *
