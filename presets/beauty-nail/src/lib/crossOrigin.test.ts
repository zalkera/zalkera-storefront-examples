 ...
 * 도달하지 않는다(실측: `!== "cross-site"` 로 바꿔도 17/17 통과 — **동치 변이**였다). 즉 형제 테넌트를
 * 실제로 막는 것은 **Origin 규칙**이고, `Sec-Fetch-Site` 는 그 위의 보조 신호일 뿐 독립적으로
 * 짐을 지지 않는다. memo118 이 경고한 것은 그 헤더를 **단독으로** 쓰는 설계였고, 우리는 안 쓴다.
 *
