 ...
 *
 * ⚠ **어느 케이스가 그 줄의 자물쇠인지 이름으로 박지 마라** — 스위트가 바뀌면 같이 낡는다. 재려면 직접 넣고 돌려라:
 *     sed -i 's/=== "same-origin"/!== "cross-site"/' src/lib/crossOrigin.ts && npm test
 * (관용구 `!== "cross-site"` 를 쓰는 설계가 위험한 것이지, 이 헤더를 **보조로** 쓰는 것 자체는 아니다.)
 *
