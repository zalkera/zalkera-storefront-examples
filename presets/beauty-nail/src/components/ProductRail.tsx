 ...
 * ## 이 파일은 고쳐 쓰라고 있는 것이다
 *
 * **중립 배선이 아니라 능력 구성이다**(memo140 §3) — 지우든 통째로 다시 짜든 자유다. 사장이
 * LLM 에게 *"여기에 인기 상품 네 개만 가로로 보여줘"* 라고 말하면 고쳐야 하는 곳이 정확히 이 한 파일이다.
 *
 * **이 파일은 쓰는 소스만 갖는다.** 팩은 각자 온전한 소스를 가지므로(오너 확정 2026-08-01) 진열을 하지
 * 않는 팩(`skeleton`·`biz-standard`)에는 이 파일 자체가 없다. 그래서 벌마다 갈려도 된다 — 진열의 디자인이
 * 갈리는 것이 이 구조의 목적이고, 갈리면 안 되는 것은 전송·인증 배선뿐이다(`scripts/lib/wiring-parity.mjs`).
 *
 * ## 지키는 규율 셋
 *
