 ...
 *
 * ⚠ **`https:` 를 통과시키지 않는 것은 빠뜨린 자리가 아니라 결정이다** — 되살리려면 인수 기준부터
 *   고쳐야 한다.
 *
 ...
 *
 * ⚠ **판정은 `mediaRef.ts` 에 있어야 이 그물이 선다.** 렌더러는 `wiring-parity` 의 바이트 잠금
 *   목록 밖이라 프리셋 한 벌만 판별자를 갈아도 아무도 못 본다 — 근거와 재현은 `bodyImageHref`
 *   KDoc 에 있다.
 */
