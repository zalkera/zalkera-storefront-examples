 ...
 * 나간다 — 계좌 객체를 넘기면 RSC 페이로드에서 계좌번호를 그대로 뽑을 수 있다.
 *
 * ⛔ **다른 게이트에는 이 형상의 그물이 없다** — 그래서 여기서 형상을 잠근다. 폼이 알아야 하는 것은
...
/**
 * 🔴 **섹션 렌더러 14종의 주소 배선도 여기서 잠근다.**
 *
 * `Markdown.tsx` 는 5벌의 주소 배선을 값으로 못박는데, `components/sections/**` 에는 따로 그물이 없다.
 * `HeroSection.tsx` 의 `assetPath(c?.asset)` 을 `asString(c?.asset)` 으로 바꾸면 **임의 제3자 `<img src>`**
 * 가 서는데, 이 시험이 없으면 `validate`·`test`·`verify` 가 그 자리를 안 잰다.
