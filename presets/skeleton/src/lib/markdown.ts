...
export const INLINE_COST: Record<Inline["kind"], number> = {
    ...
    //       (「🔴 세 축 중 몫이 먼저 닿는다」)
    //    이 값을 1 로 내리면 같은 여유를 훨씬 싸게 산다(첫 손실 24.4KB → 39.8KB · 최악 산출
    //    +2%. 몫 상한을 두 배로 올리는 길은 +19.6% 에 렌더 한 겹당 메모리도 는다).
 ...
 *
 * ⚠ **그 씨앗은 밀도가 아주 높다 — 「27KB 기사가 잘린다」로 읽지 마라.** 문단 하나가 30바이트인데
 *   그 안에 링크 1 · 강조 1 이 들어 있다. 실제 기사 문단(한국어 산문 320바이트 남짓)으로 다시
 *   재면 첫 손실은 **141~295KB** 다 — 링크가 문단마다 있고 표·목록까지 섞이면 141KB, 산문만이면
 *   291KB(그때는 몫이 아니라 블록 예산이 먼저 문다). 방향은 그대로다: 본문 상한보다 앞에서 닿는다.
 ...
 *
 * ⛔ 「산출이 커지면 ISR 캐시가 항목을 거부해 요청마다 다시 그린다」고 적혀 있었는데 **배송
 *    형상에서 재현되지 않았다** — 도달 가능한 최대 쪽(HTML 6.3MB)도 두 번째 요청부터 캐시가
 *    받았다(왕복 1회). 그 문장을 근거로 쓰지 마라.
 *
