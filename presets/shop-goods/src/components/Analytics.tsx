 ...
 * 발주처가 준 ID 가 있을 때만 열린다(랜딩 레인의 외부 의존 금지에서 이 태그만 예외다 · `docs/mockup-to-pack.md` §1-6).
 *
 ...
 *   정적 랜딩이 백엔드를 물지 않게 하기 위해서고, 그 JSON 을 쓰려는 팩이 생기면 그 팩의 레이아웃에서 따로 잇는다.
 */
const GA4_ID_PATTERN = /^G-[A-Z0-9]{4,20}$/;

export function Analytics() {
    const id = process.env.NEXT_PUBLIC_GA4_ID?.trim().toUpperCase(); // 붙여넣기 소문자를 조용히 버리지 않는다
