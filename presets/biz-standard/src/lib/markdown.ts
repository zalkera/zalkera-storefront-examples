...
export const INLINE_COST: Record<Inline["kind"], number> = {
    ...
    //    렌더러가 **요소로 안 감싸** 산출이 노드당 2바이트다 — 링크(36B)·코드(54B)와 같은 몫을
    //    무는 셈이다. 그런데 정상 기사의 몫은 **78% 가 이 `text`** 다. 재현:
    //    `node --experimental-strip-types --test src/lib/markdownRender.test.ts`
    //    (「🔴 인라인 가중치와 실제 산출」이 노드 종류별 산출을 재고 이 값을 못박는다)
    ...
    //       (「🔴 세 축 중 몫이 먼저 닿는다」)
    //    이 값을 1 로 내리면 같은 여유를 훨씬 싸게 산다(문턱 24.4KB → 39.8KB · 최악 산출 +2%,
    //    몫 상한을 두 배로 올리면 +19.6%). **고치려면 제품 결정이 필요하다** — 이 값이 바뀌면
    //    지금 잘리던 글이 안 잘린다. 아래 시험이 그 수치를 못박고 있으니 함께 움직여야 한다.
    text: 2,
    strong: 1,
    em: 1,
};
