 ...
 * ## env 이름 — `ZALKERA_*` 단독
 *
 * 읽는 이름은 `ZALKERA_*` **하나**다. 구 `ONEQUE_*` 폴백은 없다 — 서빙 인프라(백엔드 스냅샷
 * `site.env`·오케스트레이터 키 주입)가 신 이름만 방출하므로 폴백이 죽은 코드였고, 두 이름을
 * 남기면 어느 쪽이 진짜인지 흐려진다.
 *
 * **자기 인프라로 돌리는 BYO 사이트는 `.env` 의 이름을 `ZALKERA_*` 로 두어야 한다.**
 */
