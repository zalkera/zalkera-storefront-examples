...
export default async function OrderPage({
    ...
    // 남의 오입력이 쌓이면 내가 오타 한 번에 403 대신 429 를 받는다(성공한 조회는 막히지 않는다).
            ...
            <ul className="mt-4 divide-y divide-border list-none p-0">
                {order.items.map((it, i) => (
                    <li key={i} className="py-2">
                        ...
                        {accessToken && (order.status === "DELIVERED" || order.status === "COMPLETED") && it.productId != null && (
                            <ReviewForm productId={it.productId} orderItemId={it.id} />
                        )}
                    </li>
                ))}
            </ul>
